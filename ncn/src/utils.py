...
def elem2spm(element: Tensor, sizes: List[int]) -> SparseTensor:
    # convert adjancency matrix to a 1-d vector
...
def spmoverlap_notoverlap_(
    ...
    '''
    return element in adj1  but not in ajd2 and in adj2 but not adj1
    '''
    element1 = spm2elem(adj1)
    element2 = spm2elem(adj2)
...
if __name__ == '__main__':
    ...
    from ogbdataset import randomsplit
    dataset = Planetoid(root='dataset', name='Cora')
    split_edge = randomsplit(dataset)
    pos_train_edge = split_edge['train']['edge']
    pos_train_edge = pos_train_edge.t()
    print('pos_train_edge', pos_train_edge.size())
    adjmask = torch.ones_like(pos_train_edge[0], dtype=torch.bool)
    print('adjmask', adjmask, adjmask.size())

    batch_size = 128
    print(adjmask.device)

    for i, perm in enumerate(PermIterator(adjmask.device, adjmask.shape[0], batch_size)):
        print(f'{i} perm', perm.shape)

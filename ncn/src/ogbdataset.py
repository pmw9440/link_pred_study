...
def randomsplit(dataset, val_ratio: float=0.10, test_ratio: float=0.2):
    ...
    print('전치 전', torch.cat((data.train_pos_edge_index, data.val_pos_edge_index[:, :-num_val]), dim=-1) )
    print('전치 후', torch.cat((data.train_pos_edge_index, data.val_pos_edge_index[:, :-num_val]), dim=-1).t())
...
def loaddataset(name: str, use_valedges_as_input: bool, load=None):
    ...
    # Use Training + validation edges for inference on tests set.
